...
if(typesite!="mobile"&&typesite!="scroll"){
...
function adaptScrollScreen(){
if(!document.getElementById("mainbefore")){
var scrollM="<div id='scrolltoptmessage' style='top:1px;display:none;' class='noslideobject mat-shadows-a' >";
scrollM += "<div class='iconrightmessageinner rightmessageKo'></div>";
scrollM += "<div class='supprightmessage' onclick='stopdispRightMess();stopActPersoGame();'></div>";
scrollM += "<div class='flatrightmessageinner'>Réponse incorrecte !</div></div>";
$('body').append(scrollM);
$('body').append("<div id=mainbefore class=mainbefore ></div>");
$('body').append("<div id=mainafter class=mainafter ></div>");
$('#mainbefore,#mainafter').css("position","absolute");
window.addEventListener("scroll", detectScrollScreen, false);
window.addEventListener("touchmove", detectScrollScreen, false);
}
var numPg=LUDI.getNumPage();
injectScrollScreen(numPg);
var mainWidth=$("#main").width();
var mainHeight=$("#main").height();
$('#scrolltoptmessage').css("width",mainWidth+"px").css("height","52px").css("background-color","white").css("z-index","1000");
$('#scrolltoptmessage').css("position","fixed").css("left","50%").css("margin-left","-" + (mainWidth/2) + "px");
moveScrollMainScreen();
var margLeft=$("#main").css("margin-left");
margLeft=margLeft.replace("px","");
var mainTop=$("#main").offset().top;
var mainLeft=$("#main").offset().left;
$("#mainbefore,#mainafter,.mainfake").css("width",mainWidth+"px").css("left","50%").css("margin-left",margLeft+'px');
$("#mainfake").css("height",mainHeight + "px");
$("#mainbefore").css("background-color","purple");
$("#mainafter").css("background-color","yellow");
var BotH=parseInt(getWindowHeight()) - mainHeight + 150;
$("#mainafter").css("height",BotH + "px");
$("#mainbefore").css("height","8px");
$("#mainbefore").css("top","0px");
$("#mainafter").css("top",(mainTop + mainHeight) + "px");
}
function moveScrollMainScreen(){
var numPage=LUDI.getNumPage();
var mainTop=$("#main").offset().top;
var mainHeight=$("#main").height();
var Mtop=(numPage * mainHeight) + 8;
$("#main").css("top",Mtop + 'px');
$("mainfake"+numPage).css("top",Mtop + 'px');
mainTop=$("#main").offset().top;
$("#mainafter").css("top",(mainTop + mainHeight) + "px");
$("mainfake"+numPage).css("display","none");
if(numPage>0){
var prevPg=parseInt(numPage-1);
var Ptop=(prevPg * mainHeight) + 8;
$("#mainfake"+prevPg).css("position","absolute");
$("#mainfake"+prevPg).css("top",Ptop + 'px').css("left","50%");
$("#mainfake"+prevPg).css("height",mainHeight + 'px');
$("#mainfake"+prevPg).css("display","block");
}
}
function injectScrollScreen(numPg){
var numPage=LUDI.getNumPage();
if(!document.getElementById("mainfake"+numPg)){
$('body').append("<div id=mainfake"+numPg+" class=mainfake ></div>");
}
$(".mainfake").css("background-color","white");
if(numPage==numPg){
var orihtml=$("#main").html();
$("#mainfake"+numPg).html(orihtml);
for(var i=0; i < CObjets_count; i++){
var obj=CObjets[i];
$("#mainfake"+numPg).find(".bloc" + obj.id).removeClass("bloc" + obj.id).addClass("fakebloc" + obj.id)
}
$("#mainfake"+numPg).css("overflow","hidden");
$("#mainfake"+numPg).css("display","none");
}
$('body').css("touch-action","pan-y");
$('body').css("-ms-touch-action","pan-y");
$('#main').css("touch-action","pan-y");
$('#main').css("-ms-touch-action","pan-y");
}
function detectScrollScreen(){
var scrollY=window.pageYOffset || document.documentElement.scrollTop;
var mainTop=$("#mainafter").offset().top;
var mainHeight=$("#mainafter").height();
var windowHeight=parseInteger($("body").height());
var scrollBottom=parseInteger(scrollY + windowHeight + 5);
var windowReal=parseInteger(mainTop + mainHeight)-100;
$("#mainafter").html(scrollBottom + " " + windowReal);
if(scrollBottom>windowReal){
var numPage=LUDI.getNumPage();
ProcessNextPageAction(0,largEcranWidth);
var neoNumPage=LUDI.getNumPage();
if(neoNumPage!=numPage){
stopdispRightMess();
}else{
var newTop=$("#main").offset().top - 30;
...
var ctrEvol=270;
if($("#loadludi").hasClass("loadludi3")){
ctrEvol=150;
}
if(evolinit<ctrEvol){
...
if(typ=='button'||typ=='material-button'){
...
$(xml_p).find('lang').each(function(){
langue=$(this).find('d').text();
language=langue;
});
...
if(typesite=="scroll"){
$("#scrolltoptmessage").css('top',"-60px");
$("#scrolltoptmessage").css('display','block');
}else{
...
if(typesite=="scroll"){
$("#scrolltoptmessage").animate({top:"1px"}, 500);
}else{
...
if(parametersapp.indexOf('flatright')!=-1||typesite=="scroll"){
...
$('#scrolltoptmessage').animate({top:"-60px"},300, function(){
$("#scrolltoptmessage").css('display','none');
});
}
var stateHtml="";
var stateColorfond1="";
...
function getObjMemByObjId(unikId){
for(var i=0; i < CObjetMems_count; i++){
var ctr1=CObjetMems[i].idScript.toLowerCase();
var ctr2=unikId.toLowerCase();
if(ctr1==ctr2){
return CObjetMems[i];
}
}
...
textAdd=textAdd.replace('{note20}','{note}');
...
if(n>0){
note10=(n/2);
note10=Math.round(note10 * 10) / 10;
note30=(n/2)*3;
note30=Math.round(note30 * 10) / 10;
note40=(n/2)*4;
note40=Math.round(note40 * 10) / 10;
note50=(n/2)*5;
note50=Math.round(note50 * 10) / 10;
note60=(n/2)*6;
note60=Math.round(note60 * 10) / 10;
}
textAdd=textAdd.replace("{note10}",note10 + '/10');
textAdd=textAdd.replace("{note30}",note30 + '/30');
textAdd=textAdd.replace("{note40}",note40 + '/40');
textAdd=textAdd.replace("{note50}",note50 + '/50');
textAdd=textAdd.replace("{note60}",note60 + '/60');
...
objm=objm.replace("{note20}",mynote);
var note10=0;
var note30=0;
var note40=0;
var note50=0;
...
objm=objm.replace("{note10}",note10);
objm=objm.replace("{note30}",note30);
objm=objm.replace("{note40}",note40);
objm=objm.replace("{note50}",note50);
objm=objm.replace("{note60}",note60);
